...
#define FLX_DATA_PSTAMP			18
	...
	typedef struct tag_FLX_FILE_HDR
		...
		long lMilliPerFrame;			// FLC: milliseconds between frames (4 uint8_ts)
...
class CFlx
	...
	private:
		...
		int16_t WriteColorDelta(FLX_BUF* pbufNext, FLX_BUF* pbufPrev, uint8_t* pBuf, long* plChunkSize);
		int16_t WritePixelDelta(FLX_BUF* pbufNext, FLX_BUF* pbufPrev, uint8_t* pBuf, long* plChunkSize);
		int16_t WriteDataChunk(uint8_t* pData, long lSize, uint16_t wType, long* plChunkSize);
