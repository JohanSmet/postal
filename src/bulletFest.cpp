...
bool GetTerrainAngle(			// true, if terrain angle determined.
	...
	int16_t sOutAngle	= rspMod360(90 + sInAngle);	// 180 + sInAngle - 90
	...
	// Store extents.
	int16_t	sMaxX			= pRealm->GetRealmWidth();
	int16_t	sMaxZ			= pRealm->GetRealmHeight();
	...
	while (
		...
		{
		sTerrainH = m_pRealm->GetHeight((int16_t) fPosX, (int16_t) fPosZ);
		// If above terrain . . .
		if (sY > sTerrainH)
			{
			bGotOut	= true;
...
//				fPosZ		-= fRateZ;
	...
	if (bGotOut == true)
		...
		int16_t sDeltaX	= fPosX - (sX - fInRateX);
		int16_t sDeltaZ	= (sZ - fInRateZ) - fPosZ;
			...
			if (	*psX > 0 && *psX < pRealm->GetRealmWidth() 
				...
				if (sRicochets >= sMaxRicochets)
					...
					if (GetTerrainAngle(pRealm, *psX, *psY, *psZ, 10, 10, sAngleY, &sTerrainAngle) == true)
						...
						// of terrain angle . . .
						int16_t	sAngleDif	= sTerrainAngle - sAngleY;
						...
						int16_t sAngleDif	= (180 - sTerrainAngle) + sAngleY;
